...
%macro l2_dup(l = );
	...
	proc sql;
		 ...
		 select * from mil.&table.
			where %if "%lowcase(&l.)" = "y" %then not missing(cpatid) and not missing(mpatid);
  ...
  %if &ct. > 0 %then %do;
    %do i=1 %to &ct.;
	  ...
	  /* Flag [TABID]_1_xx_00-0_258: value between min and max (inclusive) */ 
		%minmaxdate(variable = &var1., dataset = %if  "%upcase(%sysfunc(substr(&var1., 1, 1)))" = "C" %then &infants.; %else &deliveries.;);
	   data flag_&i.;
        set mil.&table.;
        length message $300;
		message = "";
		flag_12= 0;
      	if not missing(&var1.) & ((&var1. gt &maxdate. | &var1. lt &mindate.)) then do;
	  	flag_12 = 1;
  ...
  %if &ct. > 0 %then %do;
	...
	%if &checkid. = 201  %then %do;
	  proc sql noprint;
		   ...
		   from mil.&&&tab1.table a left join mscdm.&&&tab2.table b
		   on a.&var1. = b.&var2.
	...
	%if &checkid. = 203 %then %do;
		...
		proc contents data = mscdm.&&&tab2.table out = l2_cont_&tab2. noprint;
		run;
	...
	%if &checkid. = 208 %then %do;
		proc sql;
			...
			and   a.&var1. ne b.&var2.
			and  not missing(a.&var1.);
		quit;
...
*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-;
*  END 01.2-mscdm-data-qa-review-level2.sas                                             ;
*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-;
